...
function ParagraphCtrl($scope, $rootScope, $route, $window, $routeParams, $location,
  ...
  $scope.getExecutionTime = function(pdata) {
    ...
    var desc = '花费 ' + moment.duration((timeMs / 1000), '秒').format('h [hrs] m [min] s [sec]') +
      '. 最后更新 ' + user + ' 于 ' + moment(pdata.dateFinished).format('MMMM DD YYYY, h:mm:ss A') + '.';
    if ($scope.isResultOutdated(pdata)) {
      desc += ' (outdated)';
    }
    return desc;
  ...
  $scope.getElapsedTime = function(paragraph) {
    return '已启动 ' + moment(paragraph.dateStarted).fromNow() + '.';
  };
